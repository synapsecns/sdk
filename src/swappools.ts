...
export type ChainSwapTypePoolsMap = ChainIdTypeMap<BridgeTokenMapping>
...
function filterGrps(chainAGrps: string[], chainBGrpsMap: SwapTypePoolTokens): Token[] {
    let tokens: Token[] = [];
...
function swapGroupsLoop(chainIdA: number, swapGrps: string[]): NetworkSwappableTokensMap {
    ...
    supportedChainIds().forEach((chainId: number) => {
        ...
        res[chainId] = filterGrps(swapGrps, bridgeSwappableMap[chainId].swappableSwapGroups);
    })
...
export function networkSwapTokensMap(chainIdA: number, chainIdB?: number): NetworkSwappableTokensMap {
    ...
    const swapGrpsA: string[] = swapGroupsForChain(chainIdA);

    if (typeof chainIdB !== 'undefined') {
        res[chainIdB] = filterGrps(swapGrpsA, bridgeSwappableMap[chainIdB].swappableSwapGroups);
    } else {
